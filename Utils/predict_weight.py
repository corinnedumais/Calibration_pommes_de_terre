...
for split_train, split_test in kfold.split(data, labels):
    ...
    # model.fit(train, train_labels, epochs=400, validation_data=(test, test_labels), batch_size=4, shuffle=True, callbacks=callbacks)
    ...
    print(f'Score for fold {fold_no}: RMSE of {np.sqrt(loss): .2f} g | MAE of {mean_abs:.2f} g | MBE {np.mean(bias):.2f} | NSE of {nash:.2f}')
...
# print(f"Real total weight: {np.sum(test_labels):.2f} g   Predicted total weight: {np.sum([i[0] for i in preds_test]):.2f} g")
#
