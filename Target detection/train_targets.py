...
input_dir = 'Target detection/TrainImages2'
target_dir = 'Target detection/TrainMasks2'
...
callbacks = [keras.callbacks.ModelCheckpoint("Trained Models/cibles_finetune3.h5", save_best_only=True)]

model = UNetST(input_size=(256, 256, 3), output_classes=1).build()
# model.load_weights('Trained Models/cibles.ckpt')
...
model = keras.models.load_model('Trained Models/cibles_finetune3.h5', custom_objects={'dice_loss': dice_loss, 'dice_coeff': dice_coeff})

### TO VISUALIZE PREDICTION ON FULL TEST IMAGE ###
# path = 'SolanumTuberosum/Test_images/test1.jpg'
# pred = full_prediction(model, path, 512, (2048, 1536))
