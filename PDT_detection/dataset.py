...
class SolanumTuberosum(keras.utils.Sequence):
    ...
    def __init__(self, batch_size: int, img_size: Tuple[int, int], input_img_paths: List[str], target_paths: List[str],
...
def GenerateDataset(directory: str):
    ...
    for k, filename in enumerate(os.listdir(os.path.join(directory, 'Images'))):
...
def generate_contour_maps(directory: str, annotations_file: str):
    ...
    for a in annotations:
        if a['regions'] and int(a['filename'][:2]) <= 12:
            ...
            mask = cv2.drawContours(mask, contours, -1, (255, 255, 255), 15)
...
# GenerateDataset('PDT_detection/Dataset Tuberosum')
#
# dir_pdt = 'PDT_detection/Dataset Tuberosum'
# #
...
#     ax = axes.ravel()
#
#     im = Image.open(f'PDT_detection/Dataset Tuberosum/TrainImages/img_{i + 1:04}.png')
#     mask = Image.open(f'PDT_detection/Dataset Tuberosum/TrainMasks/mask_{i + 1:04}.png')
#     cnt = Image.open(f'PDT_detection/Dataset Tuberosum/TrainContours/cnt_{i + 1:04}.png')
#
#     ax[0].imshow(im)
#     ax[1].imshow(mask)
#     ax[2].imshow(cnt)
